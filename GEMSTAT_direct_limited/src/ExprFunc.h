...
class DirectLimited_ExprFunc : public ExprFunc {
  public:
      // constructors
      DirectLimited_ExprFunc( const ExprModel* _model, const ExprPar& _par , const SiteVec& sites_, const int seq_len, const int seq_num) : ExprFunc( _model, _par , sites_, seq_len, seq_num){} ;
  protected:
    // compute the partition function when the BTM is bound
    double compPartFuncOn() const;
