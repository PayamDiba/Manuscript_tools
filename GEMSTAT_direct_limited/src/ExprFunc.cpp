...
double DirectLimited_ExprFunc::compPartFuncOn() const
{
    ...
    for (int k = 0; k <= N0; k++)
    {
      for ( int i = 1; i <= n; i++ )
      {
        ...
        double sum = Zt.getElement(boundaries[i],k);
        double sumA = k > 0 ? Zt.getElement(boundaries[i],k-1) : 0.0;

        for ( int j = boundaries[i] + 1; j < i; j++ )
        {
          if ( siteOverlap( sites[ i ], sites[ j ], motifs ) ) continue;
          sum += compFactorInt( sites[ i ], sites[ j ] ) * Z0.getElement(j,k);
          sumA += k > 0 ? compFactorInt( sites[i], sites[j] ) * Z0.getElement(j,k-1) : 0;
        }
        Z0.setElement(i,k,bindingWts[i] * sum);
        //Z[i] = bindingWts[ i ] * par.txpEffects[ sites[i].factorIdx ] * sum;
        if( actIndicators[ sites[ i ].factorIdx ] )
        {
          Z0(i,k) += k > 0 ? bindingWts[i] * txpEffects[sites[i].factorIdx] * sumA : 0;
          //cout << "1: " << par.txpEffects[ sites[ i ].factorIdx ] << endl;
        }
        if( repIndicators[ sites[ i ].factorIdx ] )
        {
          Z0(i,k) = bindingWts[i] * repEffects[sites[i].factorIdx] * sum;
          //cout << "2: " << par.repEffects[ sites[ i ].factorIdx ] << endl;
        }
        //cout << "DEBUG 0: " << sum << "\t" << Zt[ i - 1] << endl;
        Zt.setElement(i,k,Z0.getElement(i,k) + Zt.getElement(i - 1,k));
        /*if( actIndicators[ sites[ i ].factorIdx ] )
