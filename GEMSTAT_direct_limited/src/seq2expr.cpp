...
int main( int argc, char* argv[] )
{
    ...
    if ( seqFile.empty() || exprFile.empty() || motifFile.empty() || factorExprFile.empty() || outFile.empty() || ( ( cmdline_modelOption == QUENCHING || cmdline_modelOption == CHRMOD_UNLIMITED || cmdline_modelOption == CHRMOD_LIMITED ) &&  factorInfoFile.empty() ) || ( cmdline_modelOption == QUENCHING && repressionFile.empty() ) )
    {
        ...
        cerr << "modelOption: Logistic, Direct, Direct_Limited, Quenching, ChrMod_Unlimited, ChrMod_Limited" << endl;
        exit( 1 );
    }
    ...
    */
    ...
    //TODO: Continue here
    if( cmdline_modelOption == DIRECT || cmdline_modelOption == DIRECT_LIMITED || cmdline_modelOption == LOGISTIC || cmdline_modelOption == MARKOV || cmdline_modelOption == RATES){
